    ...
    onclick=\'$(\"#abs_{}{}\").toggle()\'>abs</a>]""".format(pub['ID'], prefix))
        ...
        if 'link' in pub:
            imgStr = "<a href=\'{}\' target='_blank'>{}</a> ".format(
                pub['link'], imgStr)
            links.append(
                "[<a href=\'{}\' target='_blank'>link</a>] ".format(pub['link']))
        if 'codeurl' in pub:
            links.append(
        ...
        if abstract:
            abstract = '''
<div id="abs_{}{}" class="my-2 bg-light-5 border rounded p-2 text-justify" style="display: none" markdown="1">
{}
</div>
'''.format(pub['ID'], prefix, abstract)

        if '_note' in pub:
            note_str = '<strong>{}</strong>'.format(pub['_note'])
        else:
            note_str = ''
...
'''.format(imgStr, author_str, title, yearVenue, note_str, links, abstract)
        else:
            return '''
<tr>
<td>
...
'''.format(author_str, title, yearVenue, note_str, links, abstract)
